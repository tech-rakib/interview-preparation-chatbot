...
def seed_default_questions():
    ...
    try:
        ...
        for topic, q_text in default_questions:
            existing = db.query(Question).filter(Question.topic == topic, Question.question_text == q_text).first()
            if not existing:
        ...
        if inserted_count > 0:
            db.commit()
            print(f"[Database] Seeded {inserted_count} new interview questions.")
